...
class DNC(Controller):
    def __init__(self, controller, batch_size, out_vector_size, mem_hp, initial_stddev = 0.1):
        """
        ...
        self.memory = Memory(self.batch_size, self.controller_output_size, self.out_vector_size, self.mem_hp, initial_stddev = initial_stddev)

        self.output_weights = tf.Variable(tf.random_normal([self.controller.out_vector_size, self.out_vector_size], stddev = initial_stddev, dtype = tf.float64),
            name = "output_weights_controller", dtype = tf.float64)

        

    def __call__(self, x, sequence_length):
        # x -- is the input data (all input cases(precedents))
        ...
        with tf.variable_scope("DNC"):
            ...
            output_initial = tf.zeros((batch_size, self.out_vector_size), dtype = tf.float64)

            all_outputs = tf.TensorArray(tf.float64, seq_len)
            # TODO remove the magic number
            all_summaries = [tf.TensorArray(tf.float64, seq_len)
    ...
    def while_loop_step(self, step, x, output, state, all_outputs, all_summaries):
        ...
        Processes one time step of DNC and keeps track of all needed variables (including initializer for visualization in tb)
        ...
        output_vector = controller_output @ self.output_weights  # @ - это умножение матриц
