...
class LSTM(Controller):
    def __init__(self, inp_vector_size, memory_size, n_layers, batch_size, out_vector_size = None, initial_stddev = None):
        ...
        if out_vector_size is not None: # Если out_vector_size не None, то...
            ...
            self.weights = tf.Variable(tf.random_normal([self.memory_size, self.out_vector_size], stddev = initial_stddev, dtype = tf.float64), name = "output_weights")
            
            self.biases = tf.Variable(tf.zeros([self.out_vector_size], dtype = tf.float64), name = "output_biases")
    ...
    def initial_state(self, batch_size):
        return self.lstm_cell.zero_state(batch_size, dtype = tf.float64)
    ...
    def __call__(self, x, sequence_lengths):
        ...
        outputs, states = tf.nn.dynamic_rnn(self.lstm_cell, x, dtype = tf.float64, sequence_length = sequence_lengths, swap_memory = True)
