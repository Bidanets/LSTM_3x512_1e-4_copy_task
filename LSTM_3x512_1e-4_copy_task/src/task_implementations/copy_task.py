...
class CopyTask(Task):
    ...
    def __init__(self, vector_size, min_seq, train_max_seq, n_copies):
        ...
        self.state = 0
        self.consecutive_thresh = 100  # порог длины последовательных случаев cost < CopyTask.epsilon
    ...
    def next_lesson(self):        # следующий урок
        ...
        if self.n_copies < 5:
            ...
            status_of_training = "Done with the training!"
        
        return status_of_training
    ...
    def update_state(self, cost):
        ...
        if self.check_lesson_learned():
            status_of_training = self.next_lesson()
    ...
    def cost(self, outputs, correct_output, mask = None):
        sigmoid_cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(logits = outputs, labels = correct_output)
    ...
    def generate_data(self, batch_size = 16, train = True, cost = 9999):
        ...
        if train:
            ...
            status_of_training = self.update_state(cost)

            self.max_seq_curriculum = self.train_max_seq
            # self.n_copies = self.max_copies
            data_batch = CopyTask.generate_n_copies(batch_size, self.vector_size, self.min_seq, self.max_seq_curriculum, self.n_copies)

        else:
            data_batch = CopyTask.generate_n_copies(batch_size, self.vector_size, self.train_max_seq, self.train_max_seq, self.n_copies)

        return data_batch + (status_of_training, )
    ...
    def generate_n_copies(batch_size, inp_vector_size, min_seq, max_seq, n_copies):
        copies_list = [CopyTask.generate_copy_pair(batch_size, inp_vector_size, min_seq, max_seq) for _ in range(n_copies)]

        output = np.concatenate([i[0] for i in copies_list], axis = 2)

        total_length = np.sum([i[1] for i in copies_list]) # суммируем длины всех копий
    ...
    def generate_copy_pair(batch_size, vector_size, min_s, max_s):
        ...
        inp_sequence = np.zeros(shape, dtype = np.float64)
        out_sequence = np.zeros(shape, dtype = np.float64)

        for i in range(batch_size):
            ones = np.random.binomial(1, 0.5, (1, sequence_length, vector_size - 1))  # бинарные векторы генерируются при помощи биномиального распределения

            inp_sequence[i, :sequence_length, :-1] = ones
            out_sequence[i, sequence_length + 1:2 * sequence_length + 1, :-1] = ones

            inp_sequence[i, sequence_length, -1] = 1  # adding the marker, so the network knows when to start copying (добавление маркера, который позволяет узнать когда начинать копировать (воспроизводить) полученную последовательность)

        """
        print('\ninp_sequence: \n', inp_sequence)
        print('\nout_sequence: \n', out_sequence)
        print('len(inp_sequence): ', len(inp_sequence[0]))
        print('len(out_sequence): ', len(out_sequence[0]))
        print(20 * "-")
        """
